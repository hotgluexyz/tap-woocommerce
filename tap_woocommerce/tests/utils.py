"""Utility functions for AWS S3 interaction and dictionary comparison.

It includes functions to load JSON data from an S3 bucket and to compare
two dictionaries for equality. The functions listed here will be moved
to the hotglue sdk in the future.

Functions:
    load_json_from_s3(prefix, file_key, logger=None):
...
def load_json_from_s3(prefix, file_key, logger=None):
    ...
    try:
        ...
        # If the configuration file is not found, attempt to load credentials
        ...
        logger.error(e)
        logger.warning(
            "Local configuration file loading failed. "
            "Loaded AWS credentials from environment variables."
        )
    ...
    response = iam_s3_client.list_objects_v2(
        Bucket=source_bucket["bucket_name"], Prefix=source_bucket["prefix"]
    )
    logger.info(
        f"Listed objects under the specified path: {source_bucket['bucket_name']}/"
    ...
    if "Contents" in response:
        ...
        for obj in response["Contents"]:
            ...
            object_key = obj["Key"]
            if not object_key.endswith(file_key):
                continue
            # Retrieve the specified object from the S3 bucket
            response = iam_s3_client.get_object(
                Bucket=source_bucket["bucket_name"], Key=object_key
            )
            logger.info(
                f"Retrieved object from S3 bucket: "
                f"{source_bucket['bucket_name']}/{object_key}"
            )

            # Read the object's content, decode it from bytes
            # to a string, and parse it as JSON
            json_content = json.loads(response["Body"].read().decode("utf-8"))
            logger.info("JSON content loaded and parsed successfully.")
            break
    else:
        logger.error(
            "No contents found under the specified path: %s/%s",
...
def compare_dicts(dict1, dict2):
    """Compare two dictionaries for equality, including nested dictionaries and lists.
